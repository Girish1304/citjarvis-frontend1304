  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* HUD Boot Diagnostics - Left Side */}
        <AnimatePresence>
          {clickCount >= 1 && (
            <motion.div
              className="absolute left-8 top-1/2 -translate-y-1/2 flex flex-col gap-3 text-xs font-mono"
              initial={{ opacity: 0, x: -50 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.5 }}
            >
              {/* System Status */}
              <div className="border border-cyan-500/30 bg-black/40 backdrop-blur-sm p-3 rounded min-w-[180px]">
                <div className="text-cyan-400/60 mb-2 tracking-wider text-[10px]">SYSTEM STATUS</div>
                {[
                  { label: 'NEURAL LINK', active: clickCount >= 1 },
                  { label: 'POWER CORE', active: clickCount >= 2 },
                  { label: 'DIAGNOSTICS', active: clickCount >= 3 },
                  { label: 'ARC REACTOR', active: clickCount >= 4 },
                  { label: 'FULL SYNC', active: clickCount >= 5 },
                ].map((item, i) => (
                  <motion.div
                    key={item.label}
                  ...
                  >
                    <span style={{ color: item.active ? 'rgba(100, 220, 255, 0.9)' : 'rgba(100, 150, 180, 0.4)' }}>
                      {item.label}
                    </span>
                    <motion.span
                      className="w-2 h-2 rounded-full"
                      style={{
                        background: item.active ? 'rgba(100, 255, 180, 0.9)' : 'rgba(80, 100, 120, 0.5)',
                        boxShadow: item.active ? '0 0 6px rgba(100, 255, 180, 0.8)' : 'none'
                      }}
              ...
              {/* Power Metrics */}
              <motion.div
                className="border border-cyan-500/30 bg-black/40 backdrop-blur-sm p-3 rounded"
                initial={{ opacity: 0 }}
                animate={{ opacity: clickCount >= 2 ? 1 : 0.3 }}
              >
                <div className="text-cyan-400/60 mb-2 tracking-wider text-[10px]">POWER METRICS</div>
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-cyan-400/50 mb-1">
                      <span>OUTPUT</span>
                      <span>{Math.round(powerLevel * 30)}GW</span>
                    </div>
                    <div className="h-1 bg-gray-800 rounded overflow-hidden">
                      <motion.div
                        className="h-full bg-gradient-to-r from-cyan-500/60 to-cyan-400/80"
                        animate={{ width: `${powerLevel}%` }}
                      />
                    </div>
                  </div>
        ...
        {/* HUD Boot Diagnostics - Right Side */}
        <AnimatePresence>
          {clickCount >= 1 && (
            <motion.div
              className="absolute right-8 top-1/2 -translate-y-1/2 flex flex-col gap-3 text-xs font-mono"
              initial={{ opacity: 0, x: 50 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.5 }}
            >
              {/* Subsystems */}
              <div className="border border-cyan-500/30 bg-black/40 backdrop-blur-sm p-3 rounded min-w-[180px]">
                <div className="text-cyan-400/60 mb-2 tracking-wider text-[10px]">SUBSYSTEMS</div>
                {[
                  { label: 'WEAPONS', value: clickCount >= 3 ? 'ARMED' : 'OFFLINE', color: clickCount >= 3 ? 'rgba(255, 180, 100, 0.9)' : 'rgba(100, 150, 180, 0.4)' },
                  { label: 'FLIGHT', value: clickCount >= 4 ? 'READY' : 'STANDBY', color: clickCount >= 4 ? 'rgba(100, 255, 180, 0.9)' : 'rgba(100, 150, 180, 0.4)' },
                  { label: 'HUD', value: 'ACTIVE', color: 'rgba(100, 220, 255, 0.9)' },
                  { label: 'COMMS', value: clickCount >= 2 ? 'LINKED' : 'INIT', color: clickCount >= 2 ? 'rgba(100, 220, 255, 0.9)' : 'rgba(100, 150, 180, 0.4)' },
                ].map((item, i) => (
                  <motion.div
                    key={item.label}
                    className="flex items-center justify-between py-1"
                    ...
                    transition={{ delay: i * 0.1 }}
                  >
                    <span className="text-cyan-400/60">{item.label}</span>
              ...
              <motion.div
                ...
                animate={{ opacity: clickCount >= 2 ? 1 : 0.3 }}
              >
                <div className="text-cyan-400/60 mb-2 tracking-wider text-[10px]">BOOT SEQUENCE</div>
                <div className="space-y-1 text-[10px]">
                  <motion.div
                    className="flex gap-2"
                    style={{ color: clickCount >= 1 ? 'rgba(100, 255, 180, 0.8)' : 'rgba(100, 150, 180, 0.3)' }}
                  >
                    <span>[OK]</span>
                  ...
                  <motion.div
                    className="flex gap-2"
                    style={{ color: clickCount >= 2 ? 'rgba(100, 255, 180, 0.8)' : 'rgba(100, 150, 180, 0.3)' }}
                  >
                    <span>{clickCount >= 2 ? '[OK]' : '[..]'}</span>
                    <span>Power routing</span>
                  </motion.div>
                  <motion.div
                    className="flex gap-2"
                    style={{ color: clickCount >= 3 ? 'rgba(100, 255, 180, 0.8)' : 'rgba(100, 150, 180, 0.3)' }}
                  >
                    <span>{clickCount >= 3 ? '[OK]' : '[..]'}</span>
                    <span>System diagnostics</span>
                  </motion.div>
                  <motion.div
                    className="flex gap-2"
                    style={{ color: clickCount >= 4 ? 'rgba(100, 255, 180, 0.8)' : 'rgba(100, 150, 180, 0.3)' }}
                  >
                    <span>{clickCount >= 4 ? '[OK]' : '[..]'}</span>
                    <span>Arc reactor sync</span>
                  </motion.div>
                  <motion.div
                    className="flex gap-2"
                    style={{ color: clickCount >= 5 ? 'rgba(100, 255, 180, 0.8)' : 'rgba(100, 150, 180, 0.3)' }}
                  >
                    <span>{clickCount >= 5 ? '[OK]' : '[..]'}</span>
                    <span>Full system online</span>
                  </motion.div>
                </div>
              </motion.div>
            </motion.div>
